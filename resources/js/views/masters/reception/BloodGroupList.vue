<template>
    <div>
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0">Blood Group Master</h4>
            <button class="btn btn-primary" @click="openModal()">
                <i class="bi bi-plus-lg me-2"></i>Add Blood Group
            </button>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr v-for="(item, index) in items" :key="item.blood_group_id">
                                <td>{{ index + 1 }}</td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ editingItem ? 'Edit Blood Group' : 'Add Blood Group' }}</h5>
                    ...
                    <form @submit.prevent="saveItem">
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" v-model="form.blood_group_name" required maxlength="10">
                            </div>
                            <div class="mb-3">
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get('/api/blood-groups', { params });
...
const openModal = (item = null) => {
    ...
    if (item) {
        form.blood_group_name = item.blood_group_name;
...
const saveItem = async () => {
    ...
    try {
        if (editingItem.value) {
            await axios.put(`/api/blood-groups/${editingItem.value.blood_group_id}`, form);
        } else {
            await axios.post('/api/blood-groups', form);
        }
        modal.hide();
        fetchData();
    } catch (error) {
...
const deleteItem = async (item) => {
    if (!confirm(`Are you sure you want to delete "${item.blood_group_name}"?`)) return;
    try {
        await axios.delete(`/api/blood-groups/${item.blood_group_id}`);
        fetchData();
    } catch (error) {
