<template>
    <div>
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0">Gender Master</h4>
            <button class="btn btn-primary" @click="openModal()">
                <i class="bi bi-plus-lg me-2"></i>Add Gender
            </button>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr v-for="(item, index) in items" :key="item.gender_id">
                                <td>{{ index + 1 }}</td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ editingItem ? 'Edit Gender' : 'Add Gender' }}</h5>
                    ...
                    <form @submit.prevent="saveItem">
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" v-model="form.gender_name" required maxlength="50">
                            </div>
                            <div class="mb-3">
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get('/api/genders', { params });
...
const openModal = (item = null) => {
    ...
    if (item) {
        form.gender_name = item.gender_name;
...
const saveItem = async () => {
    ...
    try {
        if (editingItem.value) {
            await axios.put(`/api/genders/${editingItem.value.gender_id}`, form);
        } else {
            await axios.post('/api/genders', form);
        }
        modal.hide();
        fetchData();
    } catch (error) {
...
const deleteItem = async (item) => {
    if (!confirm(`Are you sure you want to delete "${item.gender_name}"?`)) return;
    try {
        await axios.delete(`/api/genders/${item.gender_id}`);
        fetchData();
    } catch (error) {
