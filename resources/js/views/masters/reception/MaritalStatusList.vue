<template>
    <div>
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0">Marital Status Master</h4>
            <button class="btn btn-primary" @click="openModal()">
                <i class="bi bi-plus-lg me-2"></i>Add Marital Status
            </button>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr v-for="(item, index) in items" :key="item.marital_status_id">
                                <td>{{ index + 1 }}</td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ editingItem ? 'Edit Marital Status' : 'Add Marital Status' }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <form @submit.prevent="saveItem">
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label">Marital Status Name *</label>
                                <input type="text" class="form-control" v-model="form.marital_status_name" required maxlength="50">
                            </div>
                            <div class="mb-3">
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get('/api/marital-statuses', { params });
...
const openModal = (item = null) => {
    ...
    if (item) {
        form.marital_status_name = item.marital_status_name;
        form.description = item.description || '';
        form.is_active = item.is_active;
    } else {
        form.marital_status_name = '';
        form.description = '';
        form.is_active = true;
    }
    modal.show();
...
const saveItem = async () => {
    ...
    try {
        if (editingItem.value) {
            await axios.put(`/api/marital-statuses/${editingItem.value.marital_status_id}`, form);
        } else {
            await axios.post('/api/marital-statuses', form);
        }
        modal.hide();
        fetchData();
    } catch (error) {
...
const deleteItem = async (item) => {
    if (!confirm(`Are you sure you want to delete "${item.marital_status_name}"?`)) return;
    try {
        await axios.delete(`/api/marital-statuses/${item.marital_status_id}`);
        fetchData();
    } catch (error) {
