<template>
    <div>
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0">Qualification Master</h4>
            <button class="btn btn-primary" @click="openModal()">
                <i class="bi bi-plus-lg me-2"></i>Add Qualification
            </button>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr v-for="(item, index) in items" :key="item.qualification_id">
                                <td>{{ index + 1 }}</td>
                                ...
                                <td>
                                    ...
                                    <span v-else class="badge bg-info" title="In use by doctors">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ editingItem ? 'Edit Qualification' : 'Add Qualification' }}</h5>
                    ...
                    <form @submit.prevent="saveItem">
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" v-model="form.qualification_name" required maxlength="100">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Description</label>
                                <textarea class="form-control" v-model="form.description" rows="2" maxlength="255"></textarea>
                            </div>
                            <div class="mb-3">
...
const saveItem = async () => {
    ...
    try {
        if (editingItem.value) {
            await axios.put(`/api/qualifications/${editingItem.value.qualification_id}`, form);
        } else {
            await axios.post('/api/qualifications', form);
        }
        modal.hide();
        fetchData();
    } catch (error) {
...
const deleteItem = async (item) => {
    if (!confirm(`Are you sure you want to delete "${item.qualification_name}"?`)) return;
    try {
        await axios.delete(`/api/qualifications/${item.qualification_id}`);
        fetchData();
    } catch (error) {
