<template>
    <div class="container-fluid py-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="mb-0">Pathology Sensitivity Master</h4>
            <button class="btn btn-primary btn-sm" @click="openAddModal">
                <i class="bi bi-plus-circle"></i> Add Sensitivity
            </button>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-2">
                        <select class="form-select form-select-sm" v-model="filters.per_page" @change="loadSensitivities">
        ...
        <!-- Add/Edit Modal -->
        <div class="modal fade" ref="sensitivityModalRef" tabindex="-1" data-bs-backdrop="static">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ editMode ? 'Edit' : 'Add' }} Sensitivity</h5>
                        <button type="button" class="btn-close" @click="closeModal"></button>
                    </div>
                    <form @submit.prevent="saveSensitivity">
                        <div class="modal-body">
                            <div class="alert alert-danger" v-if="error">
                                {{ error }}
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Sensitivity Name *</label>
                                <input
                                    type="text"
                                    class="form-control"
                                    v-model="form.sensitivity_name"
...
let sensitivityModal = null;
const sensitivityModalRef = ref(null);
...
onMounted(async () => {
    await nextTick();
    if (sensitivityModalRef.value) {
        sensitivityModal = new Modal(sensitivityModalRef.value);
    }
    loadSensitivities();
});

const loadSensitivities = async () => {
    loading.value = true;
    error.value = null;
    try {
        ...
        error.value = 'Failed to load sensitivities';
    } finally {
        loading.value = false;
    }
};
...
const openAddModal = () => {
    ...
    if (sensitivityModal) {
        sensitivityModal.show();
    }
};

const editSensitivity = (item) => {
    editMode.value = true;
    error.value = null;
    form.value = {
...
const saveSensitivity = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || 'Failed to save sensitivity';
    } finally {
        saving.value = false;
    }
};
...
const deleteSensitivity = async (id) => {
    if (!confirm('Are you sure you want to delete this sensitivity?')) {
        return;
    }

    try {
        await axios.delete(`/api/pathology/sensitivities/${id}`);
        loadSensitivities();
    } catch (err) {
        console.error('Error deleting sensitivity:', err);
        alert(err.response?.data?.message || 'Failed to delete sensitivity');
    }
};

const closeModal = () => {
    if (sensitivityModal) {
        sensitivityModal.hide();
    }
};
</script>
